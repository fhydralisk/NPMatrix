...
bool NPMatrixCombineToRGBA(NPRGBAMap *rgba, size_t size_map,
    ...
    if (NPMATRIX_HEIGHT(rmat) != NPMATRIX_HEIGHT(gmat) || NPMATRIX_WIDTH(rmat) != NPMATRIX_WIDTH(gmat) ||
        NPMATRIX_HEIGHT(rmat) != NPMATRIX_HEIGHT(bmat) || NPMATRIX_WIDTH(rmat) != NPMATRIX_WIDTH(bmat) ||
        NPMATRIX_HEIGHT(rmat) != NPMATRIX_HEIGHT(amat) || NPMATRIX_WIDTH(rmat) != NPMATRIX_WIDTH(amat) ) {
        return false;
    }
    
    unsigned long height = NPMATRIX_HEIGHT(rmat);
    unsigned long width = NPMATRIX_WIDTH(rmat);
