...
@dynamic width,height,type,bufferSize,rawMatrix;
...
-(NSNumber *)elementAtX:(NSUInteger)x Y:(NSUInteger)y {
    ...
    if (x>=NPMATRIX_WIDTH(_matrix) || y>=NPMATRIX_HEIGHT(_matrix)) {
...
-(NPMatrixType *)rawMatrix {
    return _matrix;
}

-(NSUInteger)width {
    if (_matrix) {
        return NPMATRIX_WIDTH(_matrix);
    }
    return 0;
}
...
-(NSUInteger)height {
    if (_matrix) {
        return NPMATRIX_HEIGHT(_matrix);
    }
    return 0;
}
...
-(NSUInteger)bufferSize {
    if (_matrix) {
        return NPMATRIX_HEIGHT(_matrix) * NPMATRIX_WIDTH(_matrix) * typelen(_matrix->info.type);
    }
    return 0;
}
...
-(instancetype)modifyInRect:(NSRect)rect byUsingIntegerTypeBlock:(NPIntegerTypeBlock)block options:(NPOrderOptions)opt  {
    CHECK_NULL(_matrix);
    NPRect nprect;
    nprect.origin.x=rect.origin.x;
    nprect.origin.y=rect.origin.y;
    nprect.size.width=rect.size.width;
    nprect.size.height=rect.size.height;
    
    if (NPMatrixModifyElementInRectUsingLongTypeFunction(_matrix, nprect, long_operation, (__bridge void *)block, opt)) {
        return self;
    }
...
-(instancetype)modifyInRect:(NSRect)rect byUsingDoubleTypeBlock:(NPDoubleTypeBlock)block options:(NPOrderOptions)opt {
    CHECK_NULL(_matrix);
    NPRect nprect;
    nprect.origin.x=rect.origin.x;
    nprect.origin.y=rect.origin.y;
    ...
    if (NPMatrixModifyElementInRectUsingLDoubleTypeFunction(_matrix, nprect, double_operation, (__bridge void *)block, opt)) {
        return self;
    }
...
-(instancetype)erosionWithCore:(NPMatrix *)core originX:(NSUInteger)x originY:(NSUInteger)y {
    if (NPMatrixGraphicsErosion(_matrix, core->_matrix, x, y)) {
        return self;
    }
    return nil;
}

-(instancetype)dilationWithCore:(NPMatrix *)core originX:(NSUInteger)x originY:(NSUInteger)y {
    if (NPMatrixGraphicsDilation(_matrix, core->_matrix, x, y)) {
        return self;
    }
    return nil;
}
